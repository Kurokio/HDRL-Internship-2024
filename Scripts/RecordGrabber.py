...
""" Function List:
allRecords: Executes SQLite SELECT statements for all
            records in table
SDAC_Records: Executes SQLite SELECT statements for
                records that are published by SDAC
SPDF_Records: Executes SQLite SELECT statements for records
                that are published by SPDF
NASA_URL_Records: Executes SQLite SELECT statements for records
                    in table that have NASA URLs
"""

"""If additional tests were requested, simply create more string
variables which contain the SQLite queries to be executed and
...
class Links():
    ...
    for SPDF and SDAC, are created by joining two queries using SQLite's
    INTERSECT. This class also houses several functions which then returns
    the results from these queries. The printing is split into 3 categories:
    all records, SDAC records, and SPDF records. The results are the
    SPASE_id's of the records that match the criteria given by the SQLite
    statements.
    ...
    authorStmt = """SELECT DISTINCT SPASE_id FROM MetadataEntries
                        WHERE author NOT LIKE "" ;"""
    pubStmt = """SELECT DISTINCT SPASE_id FROM MetadataEntries
                    WHERE publisher NOT LIKE "" ;"""
    pubYrStmt = """SELECT DISTINCT SPASE_id FROM MetadataEntries
                    WHERE publicationYr NOT LIKE "" ;"""
    datasetNameStmt = """SELECT DISTINCT SPASE_id FROM MetadataEntries
                            WHERE datasetName NOT LIKE "" ;"""
    licenseStmt = """SELECT DISTINCT SPASE_id FROM MetadataEntries
                        WHERE license LIKE "%cc0%"
                        OR license LIKE
                        "%Creative Commons Zero v1.0 Universal%" ;"""
    urlStmt = """SELECT DISTINCT SPASE_id FROM MetadataEntries
                    WHERE url NOT LIKE "" ;"""
    NASAurlStmt = """SELECT DISTINCT SPASE_id FROM MetadataEntries
                        WHERE (url NOT LIKE "" AND url!="No NASA Links");"""
    PID_Stmt = """SELECT DISTINCT SPASE_id FROM MetadataEntries
                    WHERE PID NOT LIKE "" ;"""
    descStmt = """SELECT DISTINCT SPASE_id FROM MetadataEntries
    ...
    def allRecords(self, conn):
        """Executes all SQLite SELECT statements that do not have
        a specified publisher and returns the lists.
    ...
    def SDAC_Records(self):
        ...
        execution(self.SDACStmt, conn)
        execution(self.SDACauthor, conn)
        execution(self.SDACpubStmt, conn)
        execution(self.SDACpubYrStmt, conn)
        execution(self.SDACdatasetNameStmt, conn)
        execution(self.SDAClicenseStmt, conn)
        execution(self.SDACurlStmt, conn)
        execution(self.SDACNASAurlStmt, conn)
        execution(self.SDACPID_Stmt, conn)
        execution(self.SDACdescStmt, conn)
        execution(self.SDACcitationStmt, conn)
        execution(self.SDACcitationWoPID_Stmt, conn)
        execution(self.SDACcomplianceStmt, conn)
        execution(self.SDAC_AL1, conn)
    ...
    def SPDF_Records(self):
        ...
        execution(self.SPDFStmt, conn)
        execution(self.SPDFauthor, conn)
        execution(self.SPDFpubStmt, conn)
        execution(self.SPDFpubYrStmt, conn)
        execution(self.SPDFdatasetNameStmt, conn)
        execution(self.SPDFlicenseStmt, conn)
        execution(self.SPDFurlStmt, conn)
        execution(self.SPDFNASAurlStmt, conn)
        execution(self.SPDFPID_Stmt, conn)
        execution(self.SPDFdescStmt, conn)
        execution(self.SPDFcitationStmt, conn)
        execution(self.SPDFcitationWoPID_Stmt, conn)
        execution(self.SPDFcomplianceStmt, conn)
    ...
    def NASA_URL_Records(self, conn):
        ...
        NASA URLs and returns the lists.
